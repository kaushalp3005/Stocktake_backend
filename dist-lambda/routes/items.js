...
const getResultsheetMultiDate = async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: "Unauthorized" });
        }
...
exports.getResultsheetMultiDate = getResultsheetMultiDate;
...
const deleteStocktakeEntriesBySession = async (req, res) => {
    try {
        if (!req.user)
            return res.status(401).json({ error: "Unauthorized" });
        const { warehouse, floorName, enteredBy, sessionId } = req.query;
        if (!warehouse || !floorName || !enteredBy) {
            return res.status(400).json({ error: "warehouse, floorName, and enteredBy are required" });
        }
        let result;
        if (sessionId) {
            result = await prisma.$queryRawUnsafe(`DELETE FROM stocktake_entries
         WHERE warehouse = $1 AND floor_name = $2 AND entered_by = $3 AND entry_id = $4`, warehouse, floorName, enteredBy, sessionId);
        }
        else {
            result = await prisma.$queryRawUnsafe(`DELETE FROM stocktake_entries
         WHERE warehouse = $1 AND floor_name = $2 AND entered_by = $3`, warehouse, floorName, enteredBy);
        }
        // $queryRawUnsafe DELETE returns the count as a bigint
        const deleted = typeof result === "bigint" ? Number(result) : result?.count ?? 0;
...
exports.deleteStocktakeEntriesBySession = deleteStocktakeEntriesBySession;
