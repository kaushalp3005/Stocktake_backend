...
exports.getAllFloors = exports.getUserFloors = void 0;
 ...
 */
const getUserFloors = async (req, res) => {
    try {
        if (!req.user)
      ...
      `);
            ...
            rows = await prisma.$queryRawUnsafe(`SELECT DISTINCT warehouse, floor_name AS "floorName"
         FROM stocktake_entries
         WHERE entered_by = $1 OR entered_by_email = $2
         ORDER BY warehouse, floor_name`, req.user.email, req.user.email);
        }
        res.json(rows);
    }
    catch (error) {
