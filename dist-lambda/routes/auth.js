...
const login = async (req, res) => {
    ...
    `;
        ...
        if ((0, users_js_1.isBcryptHash)(dbUser.password)) {
            passwordOk = await bcryptjs_1.default.compare(String(password), dbUser.password);
        }
        else {
        ...
        if (needsUpgrade) {
            try {
                const newHash = await bcryptjs_1.default.hash(String(password), BCRYPT_ROUNDS);
...
const register = async (req, res) => {
    ...
    `;
        ...
        const hashedPassword = await bcryptjs_1.default.hash(String(password), BCRYPT_ROUNDS);
