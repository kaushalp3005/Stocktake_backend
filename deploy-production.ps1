...
try {
    ...
    # Copy built JavaScript files if they exist
    if (Test-Path "dist") {
        Copy-Item "dist/*" "lambda-production/" -Recurse -Force
        Write-Host "Copied dist files" -ForegroundColor Green
    } 
    
    # Also copy JavaScript files from root
    if (Test-Path "lambda.js") {
        Copy-Item "lambda.js" "lambda-production/" -Force
        Write-Host "Copied lambda.js" -ForegroundColor Green
    }
    ...
    # Copy all route and middleware folders
    $foldersToCopy = @("routes", "middleware", "services", "utils", "shared")
    foreach ($folder in $foldersToCopy) {
        if (Test-Path $folder) {
            Copy-Item $folder "lambda-production/" -Recurse -Force
    ...
    try {
        ...
        Write-Host "API test failed, checking with login endpoint..." -ForegroundColor Yellow
        try {
            $loginResponse = Invoke-WebRequest -Uri "https://ogj0f6xitg.execute-api.ap-south-1.amazonaws.com/prod/api/login" -Method POST -Headers @{"Content-Type"="application/json"} -Body '{"username":"test","password":"test"}' -UseBasicParsing
            Write-Host "Login Test Result: $($loginResponse.StatusCode) - $($loginResponse.Content)" -ForegroundColor Green
        } catch {
            Write-Host "Both API tests failed. Error: $($_.Exception.Message)" -ForegroundColor Red
        }
    }
