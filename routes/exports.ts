...
export const exportStocktakeEntries: RequestHandler<{}, any, {
  warehouse?: string;
  floorName?: string;
  startDate?: string;
  endDate?: string;
  ...
  try {
    ...
    let draftCountParams: any[] = [];
    if (warehouse) {
      draftCountQuery += ` AND UPPER(warehouse) = UPPER($1)`;
      draftCountParams.push(warehouse);
    }
    const draftCountResult: any[] = await prisma.$queryRawUnsafe(draftCountQuery, ...draftCountParams);
    ...
    const entries: any[] = await prisma.$queryRawUnsafe(query, ...queryParams);
    ...
    const formatDateDisplay = (d: any): string => {
      if (!d) return "";
      const dt = new Date(d);
    ...
    const verifiedCount = entries.filter((e: any) => e.verified === true).length;
    const unverifiedCount = entries.length - verifiedCount;

    // Helper function to create and populate a worksheet
    const createWorksheet = (name: string, data: any[], headerColor: string) => {
      ...
      if (draftCount > 0) subtitleParts.push(`${draftCount} draft entries excluded — not yet submitted.`);
      ...
      // Add summary footer
      const totalWeight = data.reduce((sum, entry) =>
        sum + parseFloat(entry.total_weight?.toString() || "0"), 0
      );
      const totalQuantity = data.reduce((sum, entry) =>
        sum + parseFloat(entry.total_quantity?.toString() || "0"), 0
      );
      ...
      // Unique entry users
      const uniqueEntryUsers = [...new Set(data.map((e: any) => e.entered_by).filter(Boolean))];
      // Unique verifiers
      const uniqueVerifiers = [...new Set(data.map((e: any) => e.verified_by).filter(Boolean))];

      const footerStyle = {
        font: { italic: true, size: 9 as any },
        fill: { type: "pattern" as any, pattern: "solid" as any, fgColor: { argb: "FFF4F6FA" } }
      };

      const addFooterRow = (vals: string[]) => {
    ...
    if (warehouse) filename += `_${warehouse.replace(/\s+/g, "")}`;
    if (floorName) filename += `_${floorName.replace(/\s+/g, "")}`;
    if (enteredBy) filename += `_${enteredBy.replace(/\s+/g, "")}`;
    filename += ".xlsx";
