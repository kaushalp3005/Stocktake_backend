...
const storage = multer.diskStorage({
  ...
  filename: (_req, file, cb) => {
    ...
    cb(null, `sku-${uniqueSuffix}${path.extname(file.originalname)}`);
  },
});
...
function getDbColumnName(excelColumn: string): string | null {
  ...
  workbook.eachSheet((worksheet) => {
    ...
    worksheet.eachRow((row, rowNumber) => {
      if (headerRowIndex === -1) {
        ...
        const hasParticulars = rowValues.some(
          (v) =>
            v &&
            typeof v === "string" &&
        ...
        const hasGroup = rowValues.some(
          (v) =>
            v && typeof v === "string" && normalizeColumnName(v) === "group"
        );

        if (hasParticulars || hasGroup) {
          headerRowIndex = rowNumber;
          headerRow = rowValues.map((v) =>
            v ? String(v).trim() : ""
          );
      ...
      columnIndexMap.forEach((dbColumn, index) => {
        ...
        if (value !== undefined && value !== null && value !== "") {
          ...
          if (dbColumn === "uom") {
            ...
            (parsedRow as any)[dbColumn] = strValue;
          }
        }
      });
...
export const uploadSkuFile = async (req: Request, res: Response) => {
  try {
    if (!req.user) {
    ...
    try {
      ...
      const existingRecords = await prisma.$queryRaw<
        { particulars_key: string; type_key: string }[]
      >(existingQuery);

      // Create a Set of existing keys for O(1) lookup
      const existingKeys = new Set(
        existingRecords.map(
          (r) => `${(r.particulars_key || "").toLowerCase()}_${(r.type_key || "").toLowerCase()}`
        )
      );
...
export const getSkuUploadStatus = async (req: Request, res: Response) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: "Unauthorized" });
    }
